#!/usr/bin/env bash
# Start/stop/restart DeltaX settler on VPS (nohup).
#
# Usage:
#   ./scripts/start_vps_settle.sh start
#   ./scripts/start_vps_settle.sh stop
#   ./scripts/start_vps_settle.sh status
#   ./scripts/start_vps_settle.sh restart
...
WORKER_NAME="deltax_settle"
WORKER_SCRIPT="${ROOT}/workers/deltax_settle.py"
...
Usage: $(basename "$0") {start|stop|status|restart}
  ...
  start    Launch settler (nohup, skip if already running)
